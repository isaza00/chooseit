...
class UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "2 Failed sing up due to invalid email" do
    post "/api/v1/users", params: { email: "invalid", password: "1234" }
  end
  ...
  test "13 checks creation of new user" do
    assert_difference('User.count', +1) do
      post "/api/v1/users/", params: {email: "ejemplo2@mail.com", password: "1234", name: "ejemplo2"}
    end
    assert_response 201
    user = JSON.parse(@response.body)["user"]
    assert_equal user["email"], "ejemplo2@mail.com"
    assert_equal user["name"], "ejemplo2"
  ...
  test "14 check correct update of user full_name" do
    put "/api/v1/users/100/", headers: { "Authorization": "Bearer #{@user_token}" }, params: {name: "newname"}
    assert_response :ok
    assert_equal JSON.parse(@response.body)["name"], "newname"
  end

  test "15 Tests the update method" do
    user = users :three
    assert_equal user.name, "ejemplo1"
